...
const branchData = [
...

  {
    ...
    address: "- So 22, duong Nguyen Dinh Chieu, P.Vinh Phuoc, TP.Nha Trang, Khanh Hoa"
  },
...
export default function Store() {
  const branchMap = branchData.map((data) => {
    return <Branch branch={data.branch} address={data.address} address2={data.address2}/>
  })
  return (
    <div>
      <Header />
      <div className="mt-2 min-h-screen flex justify-center items-centers flex-col">
        {branchMap}
      </div>
    </div>
  );
